...
function plot() {
    ...
    Plotly.newPlot(distribution.value, {
        data: [
            {
                x: Object.keys(props.distribution).map((labelClassId) => {
                    return props.labelClasses.find((labelClass) => labelClass.id == labelClassId)['title']
                }),
                y: Object.values(props.distribution),
                marker: {
                    color: Object.keys(props.distribution).map((labelClassId) => {
                            return convertHexToRGBA(
                                props.labelClasses.find((labelClass) => labelClass.id == labelClassId)['color'],
                            )
                        }
                    ),
                },
