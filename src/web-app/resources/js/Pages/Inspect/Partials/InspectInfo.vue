    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
            <div>
                ...
                <section class="pt-8 border-t grid grid-cols-2 gap-4">
                    <div ref="distribution" class="w-full"></div>
...
import GridCellGrid from "@/Pages/Data/Partials/GridCellGrid";
...
export default defineComponent({
    ...
    methods: {
        changeFilters(filters) {
            if (filters.label_classes) {
                this.$inertia.reload({
                    data: {
                        label_classes: filters.label_classes,
                    },
                    only: ["filters", "gridCells", "labelEvidence"],
                });
            }
        },
        ...
        createConsolidateChart() {
            ...
            Plotly.newPlot(this.$refs.distribution, {
                data: [
                    {
                        x: Object.keys(this.labelDistribution).map(
                            (label_class) => {
                                return this.labelClasses[
                                    this.labelClasses.findIndex(
                                        (data) =>
                                            data.id === parseInt(label_class)
                                    )
                                ]["name"];
                            }
                        ),
                        y: Object.values(this.labelDistribution),
                        marker: {
                            color: Object.keys(this.labelDistribution).map(
                                (label_class) => {
